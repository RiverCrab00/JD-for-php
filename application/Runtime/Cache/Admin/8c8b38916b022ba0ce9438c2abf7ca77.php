            ...
            <ul class="forminfo">
                ...
                <li>
                    ...
                    <select id="level1" class="dfinput" onchange="getCate(this.value, 2)">
                        ...
                        <?php if(is_array($cate_list)): foreach($cate_list as $key=>$vo): if($vo["cate_pid"] == 0 ): ?><option value="<?php echo ($vo["cate_id"]); ?>"><?php echo ($vo["cate_name"]); ?>
...
function getCate(cate_id,level){
    var data={'cate_id':cate_id,'_':Math.random()};
    $.get("<?php echo U('getCate');?>",data,function(msg){
        $('#level'+level).empty();
        msg="<option>--请选择--</option>"+msg;
        $('#level'+level).html(msg);
    })
}
